...
export async function reorderTabsByGroupsCommand(): Promise<void> {
  ...
  if (allTabsGroups.length === 0) {
    vscode.window.showInformationMessage(
      "No OneTab groups found. Save some tabs first!",
    );
    return;
  }
  ...
  quickPick.placeholder =
    "Choose which OneTab groups to use for organizing open tabs";
  ...
  quickPick.onDidAccept(async () => {
    ...
    if (selectedGroupIds.length > 1) {
      const shouldReorderGroups = await vscode.window.showQuickPick(
        [
          {
            label: "$(check) Use Current Order",
            description: "Apply groups in the order you selected them",
            value: false,
          },
          {
            label: "$(arrow-swap) Customize Group Order",
            description: "Manually arrange the order of groups",
            value: true,
          },
        ],
        {
      ...
      if (shouldReorderGroups.value) {
        ...
        const orderedGroupIds = await reorderGroupsInteractively(
          selectedGroupIds,
          allTabsGroups,
        );
        if (orderedGroupIds) {
