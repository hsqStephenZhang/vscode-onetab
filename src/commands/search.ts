...
export async function searchGroup() {
  const items = Array.from(
    Global.tabsProvider.getState().groups.entries(),
    (entry) => {
      return {
        groupID: entry[0],
...
export async function filterByTag() {
  ...
  if (tagCounts.size === 0) {
    vscode.window.showInformationMessage(
      "No tags found. Add tags to groups first.",
    );
    return;
  }
...
export async function filterByTagDirect(tag: string) {
  ...
  const matchingGroups = Array.from(state.groups.values()).filter((group) =>
    group.getTags().includes(tag),
  );
  ...
  if (matchingGroups.length === 1) {
    ...
    const treeview = Global.tabsProvider.getTreeView();
    await treeview?.reveal(matchingGroups[0], {
      select: false,
      focus: true,
      expand: true,
    });
