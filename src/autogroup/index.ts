...
async function addCustomStrategy(): Promise<CustomStrategy | undefined> {
  ...
  const description = await vscode.window.showInputBox({
    ...
    placeHolder:
      "e.g., Group by feature modules (auth, dashboard, settings, etc.)",
    validateInput: (value) => {
      if (!value || value.trim().length === 0) {
  ...
  vscode.window.showInformationMessage(
    `Custom strategy '${strategy.name}' saved!`,
  );
  return strategy;
}
...
async function manageCustomStrategies(): Promise<void> {
  ...
  if (selected.strategy) {
    ...
    if (action.value === "delete") {
      ...
      if (confirm === "Delete") {
        const updated = strategies.filter(
          (s) => s.name !== selected.strategy!.name,
        );
        await saveCustomStrategies(updated);
        vscode.window.showInformationMessage(
          `Strategy '${selected.strategy.name}' deleted.`,
        );
      }
      ...
      if (newDescription) {
        ...
        await saveCustomStrategies(updated);
        vscode.window.showInformationMessage(
          `Strategy '${selected.strategy.name}' updated!`,
        );
      }
    }
  }
...
export function buildPrompt(
  ...
  source: string,
  customStrategyDescriptions?: string[],
): string {
  let strategies = strategies_list
    .map((strategy) => strategies_map.get(strategy))
...
export async function requestAutoGroupWithCopilot(
  strategies_list: string[],
  filenames: string[],
  customStrategyDescriptions?: string[],
  ...
  try {
    ...
    let selectedModelId = Global.storage.getWorkspaceState(
    ...
    await Global.storage.setWorkspaceState(
      STORAGE_KEY_SELECTED_MODEL,
      selectedModelId,
    );
...
export async function requestAutoGroup(
  ...
  try {
    const copilotResult = await requestAutoGroupWithCopilot(
      strategies_list,
      filenames,
      customStrategyDescriptions,
    );
...
export async function autoGroup(): Promise<any> {
  ...
  let filenames = allTabs
    .map((file) => file.input as vscode.TabInputText)
    .map((input) => input.uri.path);
  ...
  const strategyItems: StrategyQuickPickItem[] = [
    ...
    {
      label: "file name",
      description: "Group by file name patterns",
      value: "file name",
    },
  ];
  ...
  let res = await vscode.window
    .showQuickPick(strategyItems, {
      canPickMany: true,
    ...
    .then(async (selectedItems) => {
      ...
      const hasCreate = selectedItems.some(
        (item) => item.value === "__create__",
      );
      const hasManage = selectedItems.some(
        (item) => item.value === "__manage__",
      );
      ...
      if (
        ...
        customStrategyDescriptions.length === 0
      ) {
      ...
      const result = await requestAutoGroup(
        builtInStrategies,
        filenames,
        customStrategyDescriptions,
      );
