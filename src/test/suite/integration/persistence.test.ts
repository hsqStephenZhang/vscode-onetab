...
suite("Persistence Integration Test Suite", () => {
  ...
  test("should persist and load group with tags and pinned status", async () => {
    ...
    const tab = new TabItem(
      vscode.Uri.file("/project/file.ts"),
      "tab-1",
      "file.ts",
    );
    group.setTabs([tab]);
  ...
  test("should persist and load multiple groups", async () => {
    ...
    const tab3 = new TabItem(
      vscode.Uri.file("/api/routes.ts"),
      "tab-3",
      "routes.ts",
    );
    const tab4 = new TabItem(
      vscode.Uri.file("/api/handlers.ts"),
      "tab-4",
      "handlers.ts",
    );
    ...
    const tab5 = new TabItem(
      vscode.Uri.file("/config.json"),
      "tab-5",
      "config.json",
    );
    group3.setTabs([tab5]);
  ...
  test("should persist and load state with branch name", async () => {
    ...
    const tab = new TabItem(
      vscode.Uri.file("/feature/index.ts"),
      "tab-1",
      "index.ts",
    );
    group.setTabs([tab]);
  ...
  test("should handle modifications and persist selective changes", async () => {
    ...
    assert.strictEqual(
      state3.getGroup("group-1")?.getLabel(),
      "Modified Group 1",
    );
  ...
  test("should persist with tab types (diff, notebook)", async () => {
    ...
    const diffTab = new TabItem(
      vscode.Uri.file("/diff.ts"),
      "tab-2",
      "diff.ts",
    );
  ...
  test("should isolate persistence between different branches", async () => {
    ...
    const featureTab = new TabItem(
      vscode.Uri.file("/feature.ts"),
      "tab-1",
      "feature.ts",
    );
