...
suite("Integration Workflow Test Suite", () => {
  ...
  test("should handle complete workflow: create, modify, pin, merge groups", () => {
    ...
    const tab1 = new TabItem(vscode.Uri.file("/project/src/featureA/index.ts"), "tab-1", "index.ts");
    const tab2 = new TabItem(vscode.Uri.file("/project/src/featureA/utils.ts"), "tab-2", "utils.ts");
    group1.setTabs([tab1, tab2]);
    
    const group2 = new TabsGroup("group-2", "Feature B");
    const tab3 = new TabItem(vscode.Uri.file("/project/src/featureB/main.ts"), "tab-3", "main.ts");
    group2.setTabs([tab3]);
    ...
    assert.strictEqual(state.getGroup("group-1")?.getLabel(), "Feature A - Completed");
    
    // Step 3: Add tags
    state.setGroupTags("group-1", ["completed", "reviewed"]);
    assert.deepStrictEqual(state.getGroup("group-1")?.getTags(), ["completed", "reviewed"]);
    ...
    const tab4 = new TabItem(vscode.Uri.file("/project/src/featureA/test.ts"), "tab-4", "test.ts");
  ...
  test("should handle branch archiving workflow", () => {
    ...
    const mainTab = new TabItem(vscode.Uri.file("/project/src/main.ts"), "main-tab", "main.ts");
  ...
  test("should handle removing tabs that exist in multiple groups", () => {
    ...
    const tab1 = new TabItem(vscode.Uri.file(commonFilePath), "tab-1", "common.ts");
    const tab2 = new TabItem(vscode.Uri.file("/project/src/other1.ts"), "tab-2", "other1.ts");
    group1.setTabs([tab1, tab2]);
    
    const group2 = new TabsGroup("group-2", "Group 2");
    const tab3 = new TabItem(vscode.Uri.file(commonFilePath), "tab-3", "common.ts");
    const tab4 = new TabItem(vscode.Uri.file("/project/src/other2.ts"), "tab-4", "other2.ts");
