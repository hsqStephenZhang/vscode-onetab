...
suite("Branch Switching Integration Tests", () => {
  ...
  test("Branch switch should preserve 'from' branch content completely", async () => {
    ...
    assert.strictEqual(loadedFeatureState.groups.size, 1, "Should have 1 group");
    const loadedGroup = Array.from(loadedFeatureState.groups.values())[0];
    assert.strictEqual(loadedGroup.getLabel(), "Feature Work", "Group label should be preserved");
    ...
    assert.deepStrictEqual(loadedGroup.getTags(), ["important", "feature"], "Tags should be preserved");
  });
  ...
  test("Branch switch should create independent copies", async () => {
    ...
    assert.strictEqual(reloadedGroup.getLabel(), "Main Work", "Original state should be unchanged");
  });
  ...
  test("Switching from branch A to B should preserve all tabs in branch A", async () => {
    ...
    const group1 = createTestGroup("a-group-1", "Group 1", ["/a/file1.ts", "/a/file2.ts"]);
    const group2 = createTestGroup("a-group-2", "Group 2", ["/a/file3.ts", "/a/file4.ts"]);
    ...
    const groupB = createTestGroup("b-group-1", "Branch B Work", ["/b/file1.ts"]);
    ...
    assert.strictEqual(loadedGroup1.getTabs().length, 2, "Group 1 should have 2 tabs");
    assert.strictEqual(loadedGroup2.getTabs().length, 2, "Group 2 should have 2 tabs");
    assert.strictEqual(loadedGroup3.getTabs().length, 1, "Group 3 should have 1 tab");
    
    assert.strictEqual(loadedGroup1.isPinned(), true, "Group 1 should be pinned");
    assert.deepStrictEqual(loadedGroup2.getTags(), ["work"], "Group 2 tags should be preserved");
  });
  ...
  test("Switching back and forth between branches should preserve content", async () => {
    ...
    const developGroup = createTestGroup("develop-group", "Develop", ["/dev/api.ts", "/dev/utils.ts"]);
    ...
    const hotfixGroup = createTestGroup("hotfix-group", "Hotfix", ["/hotfix/bug.ts"]);
    ...
    assert.strictEqual(loadedHotfix.groups.size, 1, "Hotfix should have 1 group");
    const hotfixGrp = Array.from(loadedHotfix.groups.values())[0];
    assert.strictEqual(hotfixGrp.isPinned(), true, "Hotfix group should be pinned");

    // Switch back from hotfix to develop
    const loadedDevelop = TabsState.loadFromStorage("develop");
    assert.strictEqual(loadedDevelop.groups.size, 1, "Develop should have 1 group");
    const devGrp = Array.from(loadedDevelop.groups.values())[0];
    assert.strictEqual(devGrp.getTabs().length, 2, "Develop group should have 2 tabs");
    assert.deepStrictEqual(devGrp.getTags(), ["backend"], "Develop tags should be preserved");
  });
  ...
  test("Deep clone should create completely independent state", () => {
    ...
    const newTab = new TabItem(vscode.Uri.file("/orig/file3.ts"), "new-tab", "file3.ts");
    ...
    assert.strictEqual(origGroup.getLabel(), "Original Group", "Original label unchanged");
    assert.deepStrictEqual(origGroup.getTags(), ["test", "original"], "Original tags unchanged");
    assert.strictEqual(origGroup.isPinned(), true, "Original pin status unchanged");
    assert.strictEqual(origGroup.getTabs().length, 2, "Original tab count unchanged");
  });
  ...
  test("State modification after cloning should not affect source", async () => {
    ...
    const sourceGroup = createTestGroup("source-group", "Source", ["/src/app.ts"]);
    ...
    const clonedGroup = Array.from(clonedState.groups.values())[0];
    const newGroup = createTestGroup("new-group", "New Group", ["/new/file.ts"]);
    ...
    assert.strictEqual(reloadedSource.groups.size, 1, "Source should still have 1 group");
    const reloadedGroup = Array.from(reloadedSource.groups.values())[0];
    assert.strictEqual(reloadedGroup.getLabel(), "Source", "Source label unchanged");
    assert.strictEqual(reloadedGroup.isPinned(), true, "Source pin status unchanged");
  });
  ...
  test("Branch state should handle empty groups correctly", async () => {
    ...
    assert.strictEqual(loadedEmpty.groups.size, 0, "Empty state should have no groups");
    assert.strictEqual(loadedEmpty.branchName, "empty", "Branch name should be preserved");
  });
  ...
  test("Multiple sequential branch switches should preserve all branches", async () => {
    ...
    for (let i = 0; i < branches.length; i++) {
      ...
      assert.strictEqual(loaded.groups.size, 1, `Branch ${i + 1} should have 1 group`);
      const group = Array.from(loaded.groups.values())[0];
      assert.strictEqual(group.getLabel(), `Group ${i + 1}`, `Branch ${i + 1} label correct`);
    }
  });
  ...
  test("Tab order should be preserved after branch switch", async () => {
    ...
    assert.ok(tabs[1].fileUri.fsPath.endsWith("second.ts"), "Second tab correct");
    assert.ok(tabs[2].fileUri.fsPath.endsWith("third.ts"), "Third tab correct");
    assert.ok(tabs[3].fileUri.fsPath.endsWith("fourth.ts"), "Fourth tab correct");
  });
  ...
  test("Complex state with multiple pinned and tagged groups should be preserved", async () => {
    ...
    const pinnedGroup = createTestGroup("pinned", "Pinned", ["/p1.ts", "/p2.ts"]);
    pinnedGroup.setPin(true);
    
    const taggedGroup = createTestGroup("tagged", "Tagged", ["/t1.ts"]);
    taggedGroup.setTags(["tag1", "tag2", "tag3"]);
    
    const pinnedTaggedGroup = createTestGroup("both", "Both", ["/b1.ts", "/b2.ts", "/b3.ts"]);
    ...
    assert.strictEqual(pinnedLoaded.isPinned(), true, "Pinned status preserved");
    assert.strictEqual(pinnedLoaded.getTabs().length, 2, "Pinned tabs count correct");
    
    assert.deepStrictEqual(taggedLoaded.getTags(), ["tag1", "tag2", "tag3"], "Tags preserved");
    
    assert.strictEqual(bothLoaded.isPinned(), true, "Both pinned status preserved");
    assert.deepStrictEqual(bothLoaded.getTags(), ["important"], "Both tags preserved");
    assert.strictEqual(bothLoaded.getTabs().length, 3, "Both tabs count correct");
    
    assert.strictEqual(regularLoaded.isPinned(), false, "Regular not pinned");
    assert.strictEqual(regularLoaded.getTags().length, 0, "Regular has no tags");
  });
});
