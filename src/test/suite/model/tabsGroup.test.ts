...
suite("TabsGroup Test Suite", () => {
  ...
  test("should create TabsGroup with default values", () => {
    ...
    assert.ok((group.label as string).startsWith(DEFAULT_TAB_GROUP_LABEL.replace(" ", "")) || 
  ...
  test("should manage tabs correctly", () => {
    ...
    const tab1 = new TabItem(vscode.Uri.file("/path/file1.ts"), "tab1", "file1.ts");
    const tab2 = new TabItem(vscode.Uri.file("/path/file2.ts"), "tab2", "file2.ts");
  ...
  test("should push tab correctly", () => {
    ...
    const tab = new TabItem(vscode.Uri.file("/path/file.ts"), "tab1", "file.ts");
  ...
  test("should extend tabs correctly", () => {
    ...
    const tab2 = new TabItem(vscode.Uri.file("/path/file2.ts"), "tab2", "file2.ts");
    const tab3 = new TabItem(vscode.Uri.file("/path/file3.ts"), "tab3", "file3.ts");
  ...
  test("should remove duplicate tabs correctly", () => {
    const group = new TabsGroup("group-id");
    const tab1 = new TabItem(vscode.Uri.file("/path/file1.ts"), "tab1", "file1.ts");
    const tab2 = new TabItem(vscode.Uri.file("/path/file1.ts"), "tab2", "file1.ts"); // duplicate path
    const tab3 = new TabItem(vscode.Uri.file("/path/file2.ts"), "tab3", "file2.ts");
