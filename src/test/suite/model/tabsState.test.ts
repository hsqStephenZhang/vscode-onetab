...
suite("TabsState Test Suite", () => {
  ...
  test("should not remove pinned group with tryRemoveTabsGroup", () => {
    const state = new TabsState();
    const group = createTestGroup("group-1", "Pinned Group", [
      "/path/file1.ts",
    ]);
    group.setPin(true);
    state.addTabsGroup(group);
  ...
  test("should remove unpinned group with tryRemoveTabsGroup", () => {
    const state = new TabsState();
    const group = createTestGroup("group-1", "Unpinned Group", [
      "/path/file1.ts",
    ]);
    state.addTabsGroup(group);
  ...
  test("should not add duplicate tabs to group", () => {
    ...
    const duplicateTab = new TabItem(
  ...
  test("should remove tab from all groups", () => {
    const state = new TabsState();
    const group1 = createTestGroup("group-1", "Group 1", [
      "/path/file1.ts",
      "/path/common.ts",
    ]);
    const group2 = createTestGroup("group-2", "Group 2", [
      "/path/file2.ts",
      "/path/common.ts",
    ]);
  ...
  test("should get pinned lists", () => {
    ...
    const unpinnedGroup = createTestGroup("unpinned", "Unpinned", [
      "/path/file2.ts",
    ]);
  ...
  test("should get titled lists", () => {
    const state = new TabsState();
    const titledGroup = createTestGroup("titled", "My Custom Name", [
      "/path/file1.ts",
    ]);
    // Use the actual constant for an untitled group
    const untitledGroup = createTestGroup(
      "untitled",
      DEFAULT_TAB_GROUP_LABEL + " 1",
      ["/path/file2.ts"],
    );
  ...
  test("should get tagged lists", () => {
    ...
    const untaggedGroup = createTestGroup("untagged", "Untagged", [
      "/path/file2.ts",
    ]);
  ...
  test("should filter groups", () => {
    const state = new TabsState();
    const pinnedTaggedGroup = createTestGroup("group-1", "Pinned Tagged", [
      "/path/file1.ts",
    ]);
    pinnedTaggedGroup.setPin(true);
    pinnedTaggedGroup.setTags(["important"]);

    const unpinnedGroup = createTestGroup("group-2", "Unpinned", [
      "/path/file2.ts",
    ]);
  ...
  test("should sort groups correctly (pinned > named > tagged > time)", () => {
    ...
    const unpinnedUntitled = createTestGroup(
      "untitled",
      DEFAULT_TAB_GROUP_LABEL + " 100",
      ["/path/file4.ts"],
    );
    const pinnedGroup = createTestGroup("pinned", "Custom Name", [
      "/path/file1.ts",
    ]);
    pinnedGroup.setPin(true);
    const namedGroup = createTestGroup("named", "Custom Name 2", [
      "/path/file2.ts",
    ]);
    // Tagged group with untitled-style name (so only tagged score applies)
    const taggedGroup = createTestGroup(
      "tagged",
      DEFAULT_TAB_GROUP_LABEL + " 200",
      ["/path/file3.ts"],
    );
  ...
  test("should deep clone state", () => {
    ...
    assert.strictEqual(
      cloned.getGroup(cloned.groups.keys().next().value!)?.getLabel(),
      "Test Group",
    );
  });
});
